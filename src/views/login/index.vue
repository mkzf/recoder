<template>
  <div class="login-container">
    ...
    <div class="version">{{ 'version:' + 'recoder-' + version }}</div>
  </div>
</template>
...
export default {
  ...
  data() {
    const validateUsername = (rule, value, callback) => {
      if (!value) {
        callback(new Error('Please fill in the correct user name'))
      } else {
        callback()
      }
    }
    const validatePassword = (rule, value, callback) => {
      if (!value) {
        callback(new Error('Please fill in the correct password'))
      } else {
        callback()
      }
    }
  ...
  methods: {
    ...
    check() {
      ...
      if (this.msg.code === 200) {
        this.$store.dispatch('websocket/sendPing', this.WsHeartbeat).then(() => {
          sessionStorage.setItem('user', JSON.stringify(this.user))
